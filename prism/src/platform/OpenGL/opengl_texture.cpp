...
: m_Path(path) {
    ...
    {
        PRISM_PROFILE_SCOPE("stbi_load - OpenGLTexture2D::OpenGLTexture2D(const std::string&)");
        data = stbi_load(path.c_str(), &width, &height, &channels, 0);
    }
    PRISM_CORE_ASSERT(data, "Failed to load image!");
    m_Width = width;
    m_Height = height;
    ...
    PRISM_CORE_ASSERT(internalFormat & dataFormat, "Format not supported!");
...
void OpenGLTexture2D::SetData(void* data, uint32_t size) {
    ...
    PRISM_CORE_ASSERT(size == m_Width * m_Height * bpp, "Data must be entire texture!");
